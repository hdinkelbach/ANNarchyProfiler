...
class ProfilerWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.ErrorbarChartTree.currentItemChanged.connect(
            self.change_errorbarchart_tree
        )
        self.ui.FunctionSelectTree.itemSelectionChanged.connect(
    ...
    def add_data(self, data):
        ...
        if data.key() in self._data:
            ...
            msg.setText(
                ...
                + " threads already exists. Want to overwrite?"
            )
    ...
    def save_chart(self):
        ...
        # tab "Proportion" selected
        elif self.ui.AnalyzerWidget.currentIndex() == 4:
            figure = self.ui.ProportionChart.figure()
        ...
        if figure != 0:
            fname = QFileDialog.getSaveFileName(
                ...
                "Save chart file",
                "./chart.png",
                "Image file (*.png *.jpg);;PDF file (*.pdf)",
            )
            if fname:
    ...
    def update_cmb_thread(self):
        ...
        for key in self._data:
            ...
            paradigm = splitted[0]
            self.ui.cmbThread.addItem(
                paradigm + " - " + thread_count + " Threads ", key
            )
    ...
    def change_multithread_selection(self):
        ...
        if parentIdx == -1:
            ...
            parentIdx = self.ui.FunctionSelectTree.invisibleRootItem().indexOfChild(
                current.parent()
            )
            ...
            for i in idx:
                mean_values.append(
                    self._data[i].values_each_test(obj_type, obj[0], obj[1], "mean")
                )
                std_values.append(
                    self._data[i].values_each_test(obj_type, obj[0], obj[1], "std")
                )
                labels.append(str(i) + " Threads")

            if self.ui.chkStdValues.isChecked():
                self.ui.MultiThreadChart.draw(
                    mean_values,
                    std_values,
                    labels,
                    yscale=str(self.ui.cmbScale.currentText()),
                )
            else:
                self.ui.MultiThreadChart.draw(
                    mean_values,
            ...
            for i in idx:
                ...
                if thread_count != "1" and paradigm_key in self._data:
                    mean_one_thread = self._data[paradigm_key].values_each_test(
                        obj_type, obj[0], obj[1], "mean"
                    )
                    mean_value = self._data[i].values_each_test(
                        obj_type, obj[0], obj[1], "mean"
                    )
            ...
            if len(mean_values) != 0:
                self.ui.SpeedupChart.draw(
                    values=mean_values,
                    labels=labels,
                    ylabel="1 Thread / x Threads",
                    yscale=str(self.ui.cmbScale.currentText()),
                )
    ...
    def change_errorbarchart_tree(self, current, previous=0):
        ...
        if current:
            ...
            if idx == -1:  # not top element?
                ...
                parentIdx = self.ui.ErrorbarChartTree.invisibleRootItem().indexOfChild(
                    current.parent()
                )
                ...
                obj = str(current.text(0)).split(" - ")
                mean_values = [
                    self.current_data().values_each_test(
                        obj_type, obj[0], obj[1], "mean"
                    )
                ]
                std_values = [
                    self.current_data().values_each_test(
                        obj_type, obj[0], obj[1], "std"
                    )
                ]

                if self.ui.chkStdValues.isChecked():
                    self.ui.ErrorbarChart.draw(
                        mean_values,
                        std_values,
                        yscale=str(self.ui.cmbScale.currentText()),
                    )
                    ...
                    self.ui.ErrorbarChart.draw(
                        mean_values, yscale=str(self.ui.cmbScale.currentText())
                    )
    ...
    def click_raw_data(self):
        ...
        if self.current_data() and self.ui.ErrorbarChartTree.selectedItems():
            ...
            if idx == -1:  # not top element?
                ...
                test_nr = self.ui.cmbRawData.itemData(
                    self.ui.cmbRawData.currentIndex()
                ).toInt()[0]
                raw_data = [
                    self.current_data().values_each_test(
                        obj_type, obj[0], obj[1], "raw"
                    )[test_nr]
                ]

                self.ui.ErrorbarChart.draw(
                    raw_data, yscale=str(self.ui.cmbScale.currentText())
                )
    ...
    def click_recalc_errorbar(self):
        ...
        if self.current_data() and self.ui.ErrorbarChartTree.selectedItems():
            ...
            if idx == -1:  # not top element?
                ...
                mean_values, std_values = self.current_data().recalc_mean_values(
                    obj_type, obj[0], obj[1], factor
                )
    ...
    def change_piechart_tree(self, current, previous):
        ...
        if current:
            ...
            if topIdx != -1:  # top element? (Network)
                ...
                overhead = data_set["step"]["mean"] - (
                    data_set["psp"]["mean"] + data_set["neur_step"]["mean"]
                )
                ...
                data = [
                    [
                        "psp\n(" + "%.4f" % data_set["psp"]["mean"] + ")",
                        "%.4f" % data_set["psp"]["mean"],
                    ],
                    [
                        "neur_step\n(" + "%.4f" % data_set["neur_step"]["mean"] + ")",
                        "%.4f" % data_set["neur_step"]["mean"],
                    ],
                ]
                ...
                topIdx = self.ui.PieChartTree.invisibleRootItem().indexOfChild(
                    current.parent()
                )
                if topIdx != -1:  # First child of Network?
                    try:
                        if childIdx == 0:
                            ...
                            neur_step = self.current_data().values_by_function(
                                topIdx, "net", "neur_step"
                            )
                            neur_step = neur_step[list(neur_step.keys())[0]]
                            func_data = self.current_data().values_by_function(
                                topIdx, "pop", "step"
                            )
                        ...
                        if childIdx == 1:
                            proj_step = self.current_data().values_by_function(
                                topIdx, "net", "proj_step"
                            )
                        ...
                        if childIdx == 2:
                            net_psp = self.current_data().values_by_function(
                                topIdx, "net", "psp"
                            )
                        ...
                        if childIdx == 3:  # rng
                            net_rng = self.current_data().values_by_function(
                                topIdx, "net", "rng"
                            )
                        ...
                        self.ui.PieChart.draw(
                            values, current.text(0) + " (in ms)", False
                        )
